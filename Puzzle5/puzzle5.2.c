...
int main(){
	...
	while(tabela[a] != 0){
		//printf("|%d::",tabela[a]);
			...
			tmp1 = &temp1;
			tmp2 = &temp2;
			tmp2->len = (tmp2->len + tabela[a]);	
			tmp1->len = (tmp1->len - tabela[a]);	
			for(int b = 0; b < tabela[a];b++){
				//printf(" %d %d ",tabela[a+1],tabela[a+2]);
				tmp2->stack[tmp2->len+b-tabela[a]] = tmp1->stack[tmp1->len+b];
				printf("tmp1:%c tmp2:%c tmp1len:%d tmp2len:%d \n",tmp1->stack[tmp1->len+b],tmp2->stack[tmp2->len+b-tabela[a]],tmp1->len+b,tmp2->len+b-1);
				tmp1->stack[tmp1->len+b] = '0';
				printf("MOVED %d::%d tmp1:%c tmp2:%c tmp1len:%d tmp2len:%d \n",tabela[a+1],tabela[a+2],tmp1->stack[tmp1->len+b],tmp2->stack[tmp2->len+b-tabela[a]],tmp1->len+b,tmp2->len+b-1);
