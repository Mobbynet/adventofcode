...
char findMatchingLetter(char *word1, char *word2){
	...
	while(word1[i] != '\0')
	{
		//printf("funckja sie wgl odpalila \n");
		j = 0;
		while(word2[j] != '\0'){
			if(word1[i] == word2[j]){
...
char** getTwoWords(char *word,int sizeOfWord_t){
	...
	for(int i =0; i < 2; ++i){
		sub_str[i] = malloc((sizeOfnWord) * sizeof(char));	
	}//for
	...
	sub_str[0][sizeOfnWord] = '\0';
	sub_str[1][sizeOfnWord] = '\0';
	 printf("%s\n%s\n", sub_str[0],sub_str[1]);
...
int main(){
	char word[] = "SHSHHlZSGZSDHGzHssGmn";
